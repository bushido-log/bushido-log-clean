#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""BUSHIDO LOG - Battle System Patch (Fixed)"""
...
battle_functions = f"""
  ...
  const executeBattleTurn = async () => {{
    ...
    const turnText = '{SWORDS} ' + dmgToEnemy + '\u30c0\u30e1\u30fc\u30b8\uff01';
    ...
    setTimeout(() => {{
      ...
      const enemyTurnText = '{FIRE} ' + battleEnemy.name + '\u306e\u53cd\u6483\uff01 ' + dmgToPlayer + '\u30c0\u30e1\u30fc\u30b8';
                ...
                <View style={{{{ flex: 1, marginLeft: 16 }}}}>
                  ...
                  <View style={{{{ flexDirection: 'row', marginTop: 6 }}}}>
                    <Text style={{{{ color: '#D4AF37', fontSize: 12 }}}}>
                      \u6226\u529b: {{'{SWORDS}'.repeat(Math.ceil(enemy.power / 25))}}
                    </Text>
                  </View>
                </View>
                <Text style={{{{ color: '#D4AF37', fontSize: 20 }}}}>{SWORDS}</Text>
              </Pressable>
            ))
          )}}
        ...
        <View style={{{{ flex: 1, padding: 20, justifyContent: 'space-between' }}}}>
          <View style={{{{ alignItems: 'center' }}}}>
            <View style={{{{ flexDirection: 'row', alignItems: 'center', marginBottom: 8 }}}}>
              {{battleEnemy.isBoss && <Text style={{{{ color: '#8B0000', fontSize: 14, marginRight: 6 }}}}>{OGRE}</Text>}}
          ...
          <View style={{{{ alignItems: 'center', marginVertical: 8 }}}}>
            <Text style={{{{ color: '#D4AF37', fontSize: 32, fontWeight: '900' }}}}>{SWORDS} VS {SWORDS}</Text>
...
# ============================================
# Find the exact closing pattern of renderCharacterTab
match = re.search(r'(      </ScrollView>\n    \);\n  \};\n)(\s*const renderAlarmTab)', content)
if match:
    btn = f"""
        {{/* Battle Button */}}
        <Pressable
        ...
        >
          <Text style={{{{ fontSize: 28, marginBottom: 4 }}}}>{SWORDS}</Text>
          <Text style={{{{ color: '#ef4444', fontSize: 20, fontWeight: '900' }}}}>\u4fee\u884c\u5bfe\u6226</Text>
          <Text style={{{{ color: '#888', fontSize: 12, marginTop: 4 }}}}>\u5df1\u306e\u529b\u3092\u8a66\u305b</Text>
          {{battleWinStreak > 0 && (
            <Text style={{{{ color: '#D4AF37', fontSize: 12, marginTop: 4 }}}}>{FIRE} {{battleWinStreak}}\u9023\u52dd\u4e2d</Text>
          )}}
        </Pressable>
"""
    insert_pos = match.start()
    content = content[:insert_pos] + btn + '\n' + content[insert_pos:]
    print('5/6 OK')
else:
    print('5/6 SKIP - button target not found')
