...
def main():
    ...
    bb.append("  const [sqIdx, setSqIdx] = useState(0);")
    bb.append("  const [sqScore, setSqScore] = useState(0);")
    bb.append("  const [sqAnswered, setSqAnswered] = useState(false);")
    bb.append("  const [sqCorrect, setSqCorrect] = useState(false);")
    bb.append("")
    ...
    bb.append("")
    # Quiz data
    bb.append("  const SQ_DATA: { [key: string]: { q: string; choices: string[]; answer: number }[] } = {")
    bb.append("    english: [")
    ...
    bb.append("  const getSqQ = () => {")
    bb.append("    const qs = SQ_DATA[selectedMission || 'english'] || SQ_DATA.english;")
    bb.append("    return qs[sqIdx] || qs[0];")
    bb.append("  };")
    bb.append("")
    # Functions
    ...
    bb.append("    } else {")
    bb.append("      setSqIdx(0); setSqScore(0); setSqAnswered(false); setSqCorrect(false);")
    ...
    bb.append("    if (sqAnswered) return;")
    bb.append("    setSqAnswered(true);")
    bb.append("    const q = getSqQ();")
    bb.append("    const correct = q.answer === choiceIndex;")
    bb.append("    setSqCorrect(correct);")
    bb.append("    if (correct) { setSqScore(sqScore + 1); try { Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success); } catch(e) {} try { Audio.Sound.createAsync(require('./sounds/taiko-hit.mp3')).then(({sound}) => sound.setVolumeAsync(MASTER_VOLUME).then(() => sound.playAsync())); } catch(e) {} }")
    bb.append("    else { try { Haptics.notificationAsync(Haptics.NotificationFeedbackType.Error); } catch(e) {} }")
    bb.append("    setTimeout(() => { if (sqIdx + 1 >= SQ_TOTAL) { onMissionComplete(); } else { setSqIdx(sqIdx + 1); setSqAnswered(false); setSqCorrect(false); } }, 1200);")
    bb.append("  };")
    bb.append("")
    ...
    ov.append("              <Text style={{ color: '#4FC3F7', fontSize: 13, letterSpacing: 2, marginBottom: 6 }}>{SQ_MISSIONS.find(m => m.id === (selectedMission || 'english'))?.label || ''}</Text>")
    ov.append("              <Text style={{ color: '#888', fontSize: 13, marginBottom: 24 }}>{'Q' + (sqIdx + 1) + ' / ' + SQ_TOTAL}</Text>")
    ...
    ov.append("                <Text style={{ color: '#fff', fontSize: 20, fontWeight: '900', textAlign: 'center', lineHeight: 30 }}>{sqQ.q}</Text>")
    ov.append("              </View>")
    ov.append("              {sqQ.choices.map((choice: string, idx: number) => (")
    ov.append("                <TouchableOpacity key={idx} onPress={() => answerQuiz(idx)} disabled={sqAnswered} style={{ backgroundColor: sqAnswered && idx === sqQ.answer ? 'rgba(46,204,113,0.3)' : 'rgba(255,255,255,0.05)', borderWidth: 1, borderColor: sqAnswered && idx === sqQ.answer ? '#2ecc71' : 'rgba(255,255,255,0.2)', borderRadius: 12, paddingVertical: 14, paddingHorizontal: 20, marginBottom: 10, width: '100%' }}>")
    ov.append("                  <Text style={{ color: sqAnswered && idx === sqQ.answer ? '#2ecc71' : '#fff', fontSize: 17, fontWeight: 'bold', textAlign: 'center' }}>{choice}</Text>")
    ov.append("                </TouchableOpacity>")
    ov.append("              ))}")
    ov.append("              {sqAnswered && (<Text style={{ color: sqCorrect ? '#2ecc71' : '#e74c3c', fontSize: 22, fontWeight: '900', marginTop: 10, letterSpacing: 2 }}>{sqCorrect ? '\u2B55 \u6B63\u89E3\uFF01' : '\u274C \u4E0D\u6B63\u89E3'}</Text>)}")
    ov.append("              <View style={{ position: 'absolute', bottom: 40, flexDirection: 'row', justifyContent: 'center' }}>")
    ov.append("                {Array.from({ length: SQ_TOTAL }).map((_, i) => (<View key={i} style={{ width: 12, height: 12, borderRadius: 6, marginHorizontal: 4, backgroundColor: i < sqIdx ? '#2ecc71' : i === sqIdx ? '#4FC3F7' : '#333' }} />))}")
    ov.append("              </View>")
    ov.append("            </View>")
    ov.append("          )}")
    ov.append("")
