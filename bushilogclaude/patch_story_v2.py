...
FIXED: missionInput -> storyGoalInput
"""
...
def main():
  ...
  const [storyGoalInput, setStoryGoalInput] = useState('');
  ...
  const submitStoryMission = async () => {
    if (storyGoalInput.trim().length < 2) return;
    try {
      const today = new Date().toISOString().split('T')[0];
            ...
            <ImageBackground source={STORY_SCENE1_IMG} style={{ flex: 1 }} resizeMode="cover">
              <View style={{ position: 'absolute', top: SCREEN_H * 0.22, left: 50, right: 50, height: SCREEN_H * 0.32, justifyContent: 'center', alignItems: 'center' }}>
                ...
                <TextInput
                  ...
                  onChangeText={setStoryGoalInput}
                  maxLength={50}
                  returnKeyType="done"
                  onSubmitEditing={submitStoryMission}
                />
                {storyGoalInput.trim().length >= 2 && (
