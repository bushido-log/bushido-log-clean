...
export const BattleScreen: React.FC<BattleScreenProps> = (props) => {
  const {
    ...
    onMissionComplete, onOugi, onRun, onClose,
        ...
        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.55)' }}>
          ...
          <ScrollView style={{ flex: 1, paddingHorizontal: 16 }} contentContainerStyle={{ paddingBottom: 40 }}>
              ...
              {phase === 'appMission' && selMission && (
                <View style={{ alignItems: 'center', padding: 20 }}>
                  <Text style={{ fontSize: 48 }}>{selMission.emoji}</Text>
                  <Text style={{ color: '#fff', fontSize: 18, fontWeight: '800', marginTop: 8 }}>{selMission.label}</Text>
                  <Text style={{ color: '#888', fontSize: 14, textAlign: 'center', marginTop: 8, lineHeight: 20 }}>
                    {selMission.desc}
                  </Text>
                  {completedMissions.includes(selMission.id) ? (
                    <Text style={{ color: '#22c55e', fontSize: 16, fontWeight: '800', marginTop: 16 }}>✅ 完了済み！</Text>
                  ) : (
                    <Text style={{ color: '#f59e0b', fontSize: 14, marginTop: 16 }}>⏳ アプリ機能を使うと自動でダメージ</Text>
                  )}
                  <Pressable onPress={() => setPhase('mission')} style={{ marginTop: 20 }}>
                    <Text style={{ color: '#888', fontSize: 13 }}>戻る</Text>
                  </Pressable>
                </View>
              )}
              ...
              {phase === 'victory' && (
                <View style={{ alignItems: 'center', paddingVertical: 16 }}>
                  ...
                  <Pressable onPress={() => { playTapSound(); onClose(); }} style={({ pressed }) => [{ backgroundColor: 'rgba(212,175,55,0.15)', borderWidth: 2, borderColor: '#D4AF37', borderRadius: 12, paddingHorizontal: 40, paddingVertical: 14, opacity: pressed ? 0.7 : 1 }]}>
