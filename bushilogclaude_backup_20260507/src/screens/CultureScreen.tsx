...
import {
  ...
  SafeAreaView, ActivityIndicator, ScrollView, Image, Linking, TextInput, Keyboard, Modal, KeyboardAvoidingView, Platform, Alert
} from 'react-native';
...
export default function CultureScreen({ onBack }: Props) {
  ...
  const fetchInfo = async (topic: string, type: string, artistData?: ArtistData) => {
    const { allowed, remaining } = checkAI('culture');
    if (!allowed) {
      setShowPaywall(true);
      return;
    }
    ...
    try {
      ...
      const data = await res.json();
      setInfo(data.reply);
      await recordUsage('culture');
      if (remaining === 1) {
        Alert.alert(lang === 'ja' ? '残り回数' : 'Usage Notice', lang === 'ja' ? '無料利用はあと1回です' : 'You have 1 free use remaining');
      }
    } catch {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <Modal visible={showLyricsModal} animationType="slide" presentationStyle="pageSheet">
        <KeyboardAvoidingView style={{ flex: 1, backgroundColor: '#0D0A05' }} behavior={Platform.OS === 'ios' ? 'padding' : undefined} keyboardVerticalOffset={90}>
    ...
    <Modal visible={showPaywall} animationType="slide">
      <PaywallScreen onClose={() => setShowPaywall(false)} screenName="culture" />
    </Modal>
    </SafeAreaView>
  );
}
