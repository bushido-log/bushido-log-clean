...
import {
  ...
  ScrollView, StyleSheet, KeyboardAvoidingView, Platform, ActivityIndicator, Alert, Modal
} from 'react-native';
...
export default function PatwaTutorScreen({ onBack }: Props) {
  ...
  const send = async () => {
    ...
    const { allowed, remaining } = checkAI('patwa');
    if (!allowed) {
      setShowPaywall(true);
      return;
    }
  ...
  return (
    ...
    <Modal visible={showPaywall} animationType="slide">
      <PaywallScreen onClose={() => setShowPaywall(false)} screenName="patwa" />
    </Modal>
    </SafeAreaView>
  );
}
